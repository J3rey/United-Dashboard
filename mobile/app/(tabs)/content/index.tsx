...
export default function Content(){
 ...
 const {content,pillars,contentFilter:filter}=data.state;
 const filtered=useMemo(()=>content.filter(c=>filter==='all'||c.pillarId===filter),[content,filter]);
 const active=useMemo(()=>filtered.filter(c=>c.status!=='Posted'),[filtered]),posted=useMemo(()=>filtered.filter(c=>c.status==='Posted'),[filtered]);
 const pillarsById=useMemo(()=>new Map(pillars.map(p=>[p.id,p])),[pillars]),selected=filter==='all'?undefined:pillarsById.get(filter),detail=content.find(c=>c.id===detailId);
 ...
 const card=(item:typeof content[number])=><IdeaCard item={item} pillar={item.pillarId===null?undefined:pillarsById.get(item.pillarId)} hidePillar={filter!=='all'} onPress={()=>setDetailId(item.id)} onDelete={()=>actions.deleteIdea(item.id)}/>;
 ...
 {selected&&<View style={[ui.line,ui.gutter,{marginBottom:10}]}><Text style={[type.subtitle,numbers,ui.flex]}>{filtered.length} {filtered.length===1?'idea':'ideas'} in {selected.name}</Text><Button label="Clear" tone="quiet" onPress={()=>select('all')}/></View>}
