...
function Ring({done,total,past}:{done:number;total:number;past:boolean}) {
 ...
 return <View style={styles.ring} accessibilityLabel={`${done} of ${total} habits satisfied`}><Svg width={46} height={46}><Circle cx={23} cy={23} r={19} stroke={colors.border} strokeWidth={5} fill="none"/><AnimatedCircle cx={23} cy={23} r={19} stroke={past?colors.amber:colors.moss} strokeWidth={5} fill="none" strokeDasharray={`${circumference} ${circumference}`} strokeLinecap="round" rotation={-90} origin="23,23" animatedProps={props}/></Svg><View style={styles.ringLabel}>{done===total?<Icon name="check" color={colors.moss} size={17}/>:<Text style={[type.label,numbers,{fontSize:11}]}>{done}/{total}</Text>}</View></View>;
}
export default function Habits() {
 ...
 <WeekStrip selected={selected} habits={habits} allHabits={data.state.habits} checks={checks} onSelect={setSelected} onWeek={direction=>changeDay(direction*7)}/>
