...
export function Chip({ label, selected, onPress, color = colors.ink }: { label: string; selected?: boolean; onPress: () => void; color?: string }) {
  return <Pressable accessibilityRole="button" accessibilityState={{ selected: Boolean(selected) }} onPress={onPress} style={[ui.chip, selected && { backgroundColor: color, borderColor: color }]}><Text numberOfLines={1} style={[ui.chipText, numbers, selected && ui.white]}>{label}</Text></Pressable>;
}
export function Sheet({ title, subtitle, children, onClose, confirmClose }: { title: string; subtitle?: string; children: ReactNode; onClose: () => void; confirmClose?: (close: () => void) => void }) {
  ...
  return <BottomSheetModal ref={modal} enableDynamicSizing maxDynamicContentSize={(height - insets.top) * 0.9} enablePanDownToClose keyboardBehavior="interactive" android_keyboardInputMode="adjustResize" overrideReduceMotion={reduced ? ReduceMotion.Always : ReduceMotion.Never} backgroundStyle={ui.sheet} handleIndicatorStyle={ui.handle} backdropComponent={props => <BottomSheetBackdrop {...props} appearsOnIndex={0} disappearsOnIndex={-1} opacity={0.4} pressBehavior="none" onPress={requestClose}/>} onDismiss={() => {
...
export function Stat({ label, value, color = colors.ink, wide = false, action }: { label: string; value: string; color?: string; wide?: boolean; action?: ReactNode }) { return <View style={[ui.stat, wide && ui.statWide]}><View style={ui.flex}><Text style={type.label}>{label}</Text><Text style={[ui.statValue, numbers, { color }, wide && ui.large]}>{value}</Text></View>{action}</View>; }
export const ui = StyleSheet.create({
  ...
  buttonText: { fontFamily: fonts.semibold, fontSize: 15, color: colors.ink2 }, white: { color: colors.surface }, red: { color: colors.red }, dim: { opacity: 0.5 },
  ...
  field: { marginBottom: 13 }, label: { fontFamily: fonts.medium, fontSize: 12, color: colors.ink2, marginBottom: 5 }, input: { minHeight: 48, borderWidth: 1, borderColor: colors.border2, backgroundColor: colors.surface, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, fontFamily: fonts.regular, fontSize: 15.5, color: colors.ink },
  ...
  stat: { flex: 1, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, borderRadius: 14, paddingHorizontal: 13, paddingVertical: 11 }, statWide: { flex: 0, flexDirection: 'row', alignItems: 'center', width: '100%', marginBottom: 9 }, statValue: { fontFamily: fonts.semibold, fontSize: 21, letterSpacing: -0.63 }, large: { fontSize: 27, letterSpacing: -0.81 },
});
