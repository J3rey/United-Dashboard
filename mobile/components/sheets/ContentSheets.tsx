...
export function IdeaDetailSheet({item,onClose}:{item:ContentItem;onClose:()=>void}) {
 ...
 <Field label="Notes · optional" multiline style={{minHeight:100,textAlignVertical:'top',marginTop:12}} value={notes} onChangeText={setNotes} onBlur={()=>{if(notes!==item.notes)void actions.editIdea(item.id,{notes});}}/>{next&&<Button label={`Mark as ${next.toLowerCase()}`} disabled={actions.busy} onPress={async()=>{if(await actions.editIdea(item.id,{status:next}))onClose();}}/>}<Button label="Delete idea" tone="danger" disabled={actions.busy} onPress={()=>Alert.alert('Delete idea?','You can undo this for four seconds.',[{text:'Cancel',style:'cancel'},{text:'Delete',style:'destructive',onPress:()=>{actions.deleteIdea(item.id);onClose();}}])}/><Button label="Done" tone="quiet" onPress={onClose}/></Sheet>
