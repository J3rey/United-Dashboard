...
export function LedgerRow({ expense, onPress, onDelete, first, last }: { expense: Expense; onPress: () => void; onDelete: () => void; first?: boolean; last?: boolean }) {
  return <SwipeRow label={expense.detail} onDelete={onDelete}><Pressable accessibilityRole="button" accessibilityLabel={`${expense.detail}, ${money(expense.cost)}, ${expense.cat}`} onPress={onPress} style={[styles.row, first && styles.first, last && styles.last]}>
