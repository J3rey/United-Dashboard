...
function dateFromWeekStartAndDay(weekStart: string, dayIndex: number) {
  const d = new Date(weekStart + 'T12:00:00')
  d.setDate(d.getDate() + dayIndex)
  return toDs(d)
}

function weekStartAndDayFromDate(dateStr: string) {
...
export async function insertTransaction(userId: string, tx: Transaction, sortOrder: number): Promise<Id> {
  const row: Database['public']['Tables']['finance_transactions']['Insert'] = tx.isHeader
...
export async function updateTransaction(id: Id, changes: TransactionChanges) {
  const mapped: TransactionChanges & { tx_type?: TransactionRow['tx_type'] } = { ...changes }
  if ('type' in mapped) { mapped.tx_type = mapped.type; delete mapped.type }
  const { error } = await supabase.from('finance_transactions').update<Database['public']['Tables']['finance_transactions']['Update']>(mapped).eq('id', id)
  if (error) throw error
}
...
export async function fetchIncome(userId: string): Promise<Income[]> {
  const { data, error } = await supabase
    .from('finance_income')
    .select('*')
    .eq('user_id', userId)
...
export async function insertIncome(userId: string, inc: Income): Promise<Id> {
  const { data, error } = await supabase
    .from('finance_income')
...
export async function updateIncome(id: Id, changes: IncomeChanges) {
  const mapped: Database['public']['Tables']['finance_income']['Update'] = {}
...
export async function insertHabit(userId: string, habit: Habit, sortOrder: number): Promise<Id> {
  const { data, error } = await supabase
    .from('habits')
...
export async function updateHabit(id: Id, changes: HabitChanges) {
  const mapped: Database['public']['Tables']['habits']['Update'] = {}
...
export async function fetchHabitLogs(userId: string): Promise<Record<string, boolean>> {
  const { data, error } = await supabase
    .from('habit_logs')
    .select('*')
    .eq('user_id', userId)
...
export async function toggleHabitLog(userId: string, habitId: Id, dateStr: string, checked: boolean) {
...
export async function fetchContent(userId: string): Promise<{ pillars: Pillar[]; content: ContentItem[] }> {
...
export async function insertPillar(userId: string, pillar: Pillar, sortOrder: number): Promise<Id> {
  const { data, error } = await supabase
...
export async function deletePillar(id: Id) {
...
export async function updateContentOrder(items: Pick<ContentItem, 'id'>[]) {
...
export async function deleteContentItem(id: Id) {
